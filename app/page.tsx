        ...
        <section className="hero-section section-wrap" id="invitation">
          <div className="hero-grid">
            <div className="hero-copy reveal-item">
              <div className="eyebrow"><span className="eyebrow-line" /> СЕКРЕТНОЕ ПРИГЛАШЕНИЕ / 001</div>
            ...
            <div className="hero-terminal reveal-item reveal-delay-1" aria-label="Статус секретной операции">
              <div className="terminal-topline"><span>КОНТРОЛЬ МИССИИ</span><span>ЗАШИФРОВАНО</span></div>
              <div className="terminal-body">
                <div className="terminal-ascii" aria-hidden="true">{`╔══════════════════╗
║ ДОСТУП РАЗРЕШЁН ║
╚══════════════════╝`}</div>
                <div className="terminal-readout">
                  <p><span className="dim">АГЕНТ //</span> {eventConfig.groomName.toUpperCase()}</p>
                  <p><span className="dim">ОПЕРАЦИЯ //</span> ПОСЛЕДНИЙ ВЕЧЕР СВОБОДЫ</p>
                  <p><span className="dim">СТАТУС //</span> <span className="ok">ПОДТВЕРЖДЕНО</span></p>
                  <p><span className="dim">РИСК //</span> <span className="warn">КРИТИЧЕСКИЙ</span></p>
                </div>
                <div className="terminal-progress"><span>ОПЕРАЦИЯ ГОТОВА</span><b><i /></b><span>100%</span></div>
              </div>
              <div className="terminal-footer">ИД СЕССИИ: BP-0912 / НЕ ПЕРЕСЫЛАТЬ</div>
            </div>
          </div>
        </section>

        <section className="section-wrap briefing-section" id="briefing">
          <div className="section-heading"><span className="section-number">01</span><div><p className="eyebrow">ЛИЧНОЕ ДЕЛО</p><h2>Досье объекта</h2></div></div>
          <InvitationDossier />
        </section>

        <section className="section-wrap countdown-section">
          <div className="countdown-card">
            <div className="countdown-label"><span className="live-dot" /> МИССИЯ НАЧИНАЕТСЯ ЧЕРЕЗ</div>
            <Countdown />
        ...
        <section className="section-wrap timeline-section" id="timeline">
          <div className="section-heading"><span className="section-number">02</span><div><p className="eyebrow">ПЛАН ОПЕРАЦИИ</p><h2>План операции</h2></div></div>
          <MissionTimeline />
        </section>

        <section className="section-wrap location-section" id="location">
          <div className="section-heading"><span className="section-number">03</span><div><p className="eyebrow">ДАННЫЕ ГЕОЛОКАЦИИ</p><h2>Координаты сбора</h2></div></div>
          <LocationMap />
        </section>

        <section className="section-wrap final-section" id="accept">
          <div className="final-grid">
            <div><p className="eyebrow">ОБРАТНОЙ ДОРОГИ НЕТ / 04</p><h2>Готов к<br /><span>выходу?</span></h2><p className="final-copy">Система зафиксирует твой ответ и передаст его командованию.</p></div>
            <div className="final-actions"><AddToCalendar /><AcceptMission /></div>
          </div>
        </section>

        <footer className="site-footer section-wrap">
          <div><span className="brand-mark">⌁</span> ЧЁРНЫЕ ОПЕРАЦИИ / МАЛЬЧИШНИК</div>
          <button className="restart-button" onClick={restartIntro}>↻ ПОВТОРИТЬ ВВОД</button>
          <div className="footer-meta">СОЗДАНО ДЛЯ ОДНОЙ НОЧИ <span>◆</span></div>
        </footer>

        {easterEgg && <div className="easter-egg" role="status"><span>&gt;</span> БОНУСНАЯ ЦЕЛЬ ОТКРЫТА: ДОВЕСТИ ЖЕНИХА ДО УТРА <button onClick={() => setEasterEgg(false)} aria-label="Закрыть сообщение">×</button></div>}
